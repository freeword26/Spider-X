...
logger = logging.getLogger("worker-cluster.sop")
...
class SOPEngine:
    ...
    async def execute_pipeline(self, pipeline_id: str, initial_input: Dict[str, Any] = None) -> SOPPipeline:
        ...
        pipeline.status = SOPStatus.RUNNING
        ...
        while len(completed) + len(failed) < len(pipeline.steps):
            ...
            if not ready_steps:
                if any(s.status == SOPStatus.RUNNING for s in pipeline.steps):
        ...
        if failed:
            pipeline.status = SOPStatus.FAILED
            pipeline.error = f"{len(failed)} step(s) failed"
        else:
            pipeline.status = SOPStatus.COMPLETED
    ...
    def _get_ready_steps(self, pipeline: SOPPipeline, completed: set, failed: set) -> List[SOPStep]:
        ...
        for step in pipeline.steps:
            if step.status != SOPStatus.PENDING:
                continue
